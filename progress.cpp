...
char masterPass[_MAX_PATH] = "C:\\work\\test\\progress\\";
...
int main(void) {
	...
	/*パスを用意*/
	char pass1[_MAX_PATH] = "C:\\work\\test\\progress\\";
	char yearPath[5];
	char pass2[] = "\\";
	char monthPath[3];
	char pass3[] = "\\";
	...
	HANDLE hFind, d_hFind;
	WIN32_FIND_DATA fd, d_fd;
	FILETIME ft;
	SYSTEMTIME st;
	...
	if(d_hFind == INVALID_HANDLE_VALUE) {
		...
		printf("既存のフォルダを利用します\n");
		/* ディレクトリが存在する場合、古いファイルが中にあれば削除する */
		sprintf(findPass, "%s%04d%02d%02d*.scd", masterCpPass, n_year, n_month, n_day);
		d_hFind = FindFirstFile(findPass, &d_fd);
		/* 検索ファイルが見つからない場合 */
		if(d_hFind == INVALID_HANDLE_VALUE) {
			printf("削除該当ファイル無し\n\n");
			//何もせずに終了
