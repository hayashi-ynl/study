// sensor2.cpp : コンソール アプリケーションのエントリ ポイントを定義します。
// 2017-05-18 20:44
...
char KMPmasterCpPass[_MAX_PATH] = "C:\\iot_work\\MiniFA-KMP\\Censor\\KMP\\";	//処理結果ディレクトリ(KMP)

char vip_in[] = "sensor06-punch.csv";
int vip_in_last;
char vip_out[] = "sensor13-punch.csv";
int vip_out_last;
char kmp_in[] = "sensor06-bend.csv";
int kmp_in_last;
char kmp_out[] = "sensor13-bend.csv";
int kmp_out_last;
...
int main(void) {
	...
	/* KMP */
	sprintf(write_Pass, "%s%04d%02d%02d-KMP.cen", VIPmasterCpPass, n_year, n_month, n_day);

	read_csv(KMP_IN);
	read_csv(KMP_OUT);

	last_i = calculation(kmp_in_last, kmp_out_last);
...
void read_csv(unsigned char sw)
{
	...
	if(sw == VIP_IN){
		...
		sprintf(tmp_pass, "%s%s", d_Pass, kmp_in);
	}else if(sw == KMP_OUT){
		sprintf(tmp_pass, "%s%s", d_Pass, kmp_out);
	}
	...
	for (row = 1; fgets(read_s, sizeof(read_s), dt_fp) != NULL; row++) {
		if (row == 1) {  /*最初の行*/
			...
			for (col = 1; tok != NULL; col++) {
				if(col == 1){ //1列目
					//選択処理(VIP_IN　VIP_OUT KMP_IN KMP_OUT)
					if(sw == VIP_IN || sw == KMP_IN){
						strcpy(&dt_in_org[i][0], tok);
					}else if(sw == VIP_OUT || sw == KMP_OUT){
